...
import com.ke.dawaati.widgets.reader.gestures.Compat
import com.ke.dawaati.widgets.reader.gestures.OnGestureListener
import com.ke.dawaati.widgets.reader.gestures.VersionedGestureDetector
import com.ke.dawaati.widgets.reader.log.LogManager
import com.ke.dawaati.widgets.reader.scrollerproxy.ScrollerProxy
import java.lang.ref.WeakReference
import kotlin.math.pow
import kotlin.math.sqrt

class PhotoViewAttacher(imageView: ImageView) : IPhotoView, OnTouchListener, OnGestureListener, OnGlobalLayoutListener {
    var ZOOM_DURATION: Int = IPhotoView.DEFAULT_ZOOM_DURATION
    private var mMinScale: Float = IPhotoView.DEFAULT_MIN_SCALE
    private var mMidScale: Float = IPhotoView.DEFAULT_MID_SCALE
    private var mMaxScale: Float = IPhotoView.DEFAULT_MAX_SCALE
    private var mAllowParentInterceptOnEdge = true
    private var mImageView: WeakReference<ImageView>?

    // Gesture Detectors
    private val mGestureDetector: GestureDetector?
    private val mScaleDragDetector: com.ke.dawaati.widgets.reader.gestures.GestureDetector?
    ...
    override fun setOnDoubleTapListener(newOnDoubleTapListener: GestureDetector.OnDoubleTapListener?) {
        if (newOnDoubleTapListener != null) mGestureDetector!!.setOnDoubleTapListener(newOnDoubleTapListener) else mGestureDetector!!.setOnDoubleTapListener(newOnDoubleTapListener)
    }
    ...
    override var scale: Float
        get() = sqrt(getValue(mSuppMatrix, Matrix.MSCALE_X).toDouble().pow(2.0).toFloat() + getValue(mSuppMatrix, Matrix.MSKEW_Y).toDouble().pow(2.0)).toFloat()
        set(scale) {
    ...
    private fun checkImageViewScaleType() {
        ...
        if (null != imageView && imageView !is IPhotoView) {
            check(ScaleType.MATRIX == imageView.scaleType) { "The ImageView's ScaleType has been changed since attaching a PhotoViewAttacher" }
        }
    }
    ...
    private fun updateBaseMatrix(d: Drawable?) {
        ...
        if (mScaleType == ScaleType.CENTER) {
            ...
            val mTempSrc = RectF(0.0f, 0.0f, drawableWidth.toFloat(), drawableHeight.toFloat())
            val mTempDst = RectF(0.0f, 0.0f, viewWidth, viewHeight)
            when (mScaleType) {
    ...
    companion object {
        private const val LOG_TAG = "PhotoViewAttacher"

        // let debug flag be dynamic, but still Proguard can be used to remove from
        // release builds
        private val DEBUG = Log.isLoggable(LOG_TAG, Log.DEBUG)
        val sInterpolator: Interpolator = AccelerateDecelerateInterpolator()
        ...
        private fun isSupportedScaleType(scaleType: ScaleType?): Boolean {
            return if (null == scaleType) {
                ...
                ScaleType.MATRIX -> throw IllegalArgumentException(
                    scaleType.name +
